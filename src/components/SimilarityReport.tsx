...
const MOCK_CODE = `import java.util.Scanner;
...
export default function SimilarityReport() {
  ...
  if (loading) return (
    <div style={{ minHeight: "100vh", background: "#050505", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ color: "#71717a", fontSize: 14 }}>Loading report...</div>
    </div>
  );

  if (error || !data || data.error) return (
    <div style={{ minHeight: "100vh", background: "#050505", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ color: "#ef4444", fontSize: 14 }}>No MOSS report found for this project yet.</div>
    </div>
  );

  const matches = data.matches || [];
  const TOTAL = MOCK_CODE.length;
  ...
  const totalMatched = (() => {
    const all = matches.flatMap((m: any) => (m.segments || []).filter((s: any) => s.side === "A").map((s: any) => ({ start: s.startLine, end: s.endLine }))).sort((a: any, b: any) => a.start - b.start);
  ...
  const tips = [
    { icon: "✏️", title: "Rename variables meaningfully", desc: "Replace arr, n, i with descriptive names." },
    ...
    { icon: "🛡️", title: "Handle edge cases differently", desc: "Add input validation that shows deep understanding." },
        ...
        <div style={{ background: "#0f0f0f", border: "1px solid #1e1e1e", borderRadius: 12, padding: "16px 20px" }}>
          ...
          <div style={{ fontFamily: "monospace", fontSize: 12, lineHeight: "1.7", background: "#080808", border: "1px solid #1e1e1e", borderRadius: 10, overflow: "auto", maxHeight: 600 }}>
            {MOCK_CODE.map((line, i) => {
              ...
              const c = idx >= 0 ? COLORS[idx % 4] : null;
              return (
