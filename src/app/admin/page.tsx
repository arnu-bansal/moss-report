...
export default function AdminPage() {
  ...
  const [allowSnippet, setAllowSnippet] = useState(false);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#050505", color: "#d4d4d4", fontFamily: "system-ui, sans-serif" }}>
      <div style={{ borderBottom: "1px solid #1a1a1a", padding: "14px 24px", display: "flex", alignItems: "center", gap: 14 }}>
        <div style={{ width: 28, height: 28, borderRadius: 6, background: "linear-gradient(135deg,#6366f1,#a855f7)", display: "flex", alignItems: "center", justifyContent: "center" }}>
          ⚙
        </div>
        <span style={{ fontWeight: 700, fontSize: 15, color: "#f5f5f5" }}>CodeScan Admin</span>
        <span style={{ marginLeft: "auto", fontSize: 10, background: "#1e1b4b", color: "#818cf8", padding: "3px 10px", borderRadius: 99 }}>
          ADMIN
        </span>
      </div>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "24px 20px" }}>
        <div style={{ background: "#0a0a14", border: "1px solid #1e1e3a", borderRadius: 12, padding: "24px", marginBottom: 24 }}>
          ...
          <div style={{ fontSize: 12, color: "#4f4f6a", marginBottom: 16 }}>
            Triggers a new MOSS check for project-1.
          </div>
          <button
            onClick={triggerMoss}
            disabled={running}
            style={{ padding: "12px 28px", borderRadius: 8, border: "1px solid #4f46e5", background: "#1e1b4b", color: "#c7d2fe", fontWeight: 700, fontSize: 13, cursor: running ? "not-allowed" : "pointer" }}
          >
        ...
        <div style={{ background: "#0a0a14", border: "1px solid #1e1e3a", borderRadius: 12, padding: "24px", marginBottom: 24 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
            <div style={toggleStyle(revealIdentity)} onClick={() => setRevealIdentity((v) => !v)}>
              <div style={knobStyle(revealIdentity)} />
            </div>
            <span style={{ fontSize: 13, color: "#a5b4fc" }}>Reveal counterpart identity to students</span>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <div style={toggleStyle(allowSnippet)} onClick={() => setAllowSnippet((v) => !v)}>
              <div style={knobStyle(allowSnippet)} />
            </div>
            <span style={{ fontSize: 13, color: "#a5b4fc" }}>Allow showing counterpart snippet</span>
          </div>
        </div>
          ...
          {loading ? (
            ...
            runs.map((r: any) => (
              <div key={r.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", background: "#0d0d1a", borderRadius: 8, marginBottom: 8, border: "1px solid #1e1e3a" }}>
                <span style={{ width: 8, height: 8, borderRadius: "50%", background: r.status === "COMPLETED" ? "#4ade80" : "#ef4444", flexShrink: 0 }} />
                <span style={{ fontFamily: "monospace", fontSize: 12, color: "#64748b" }}>
                  {new Date(r.createdAt).toLocaleString()}
                </span>
                <span style={{ fontSize: 12, color: "#818cf8" }}>{r.projectId}</span>
                <span style={{ marginLeft: "auto", fontSize: 11, color: r.status === "COMPLETED" ? "#4ade80" : "#ef4444" }}>
                  {r.status}
                </span>
              </div>
            ))
