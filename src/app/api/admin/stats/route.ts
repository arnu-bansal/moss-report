...
export async function GET(req: NextRequest) {
  try {
    ...
    const [users, projects, submissions, runs] = await Promise.all([
      ...
      prisma.mossRun.findMany({ orderBy: { createdAt: "desc" }, take: 50, select: { id: true, projectId: true, status: true, triggeredBy: true, createdAt: true, project: { select: { name: true, language: true } }, _count: { select: { matches: true } } } }),
    ]);
